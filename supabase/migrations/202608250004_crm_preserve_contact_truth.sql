-- A human takeover proves ownership only. It does not prove that contact
-- happened, so HUMAN_NEW remains in SALES_INBOX until an explicit move.

drop trigger if exists crm_protect_board_column_identity on public.crm_board_columns;

update public.crm_board_columns
set stage_keys = array['AI_CALL_REQUESTED','HUMAN_NEW']::public.crm_lead_stage[],
    updated_at = now()
where board = 'SALES' and column_key = 'SALES_INBOX';

update public.crm_board_columns
set stage_keys = array['HUMAN_CONTACTING']::public.crm_lead_stage[],
    updated_at = now()
...
begin
  insert into public.crm_board_columns(
    ...
    (p_organization_id, 'SALES', 'SALES_INBOX', array['AI_CALL_REQUESTED','HUMAN_NEW']::public.crm_lead_stage[], 'Lead entrante', '#6E8FBA', 0, true),
