...
import type {
  CrmActivityDTO,
  CrmCommercialProfileDTO,
  CrmConversationMessageDTO,
  CrmLeadWorkspaceDTO,
  CrmTaskStatus,
} from "@/lib/crm-workspace-model"
...
interface CrmConversationRow {
  message_id: number
  speaker: CrmConversationMessageDTO["speaker"]
  content: string
...
export async function getCrmLeadWorkspace(leadId: string): Promise<CrmLeadWorkspaceDTO> {
  ...
  const [commercialResult, conversationResult, activitiesResult, notesResult, tasksResult] = await Promise.all([
    supabase.rpc("crm_get_lead_commercial_profile", { p_lead_id: leadId }),
    supabase.rpc("crm_get_lead_conversation", { p_lead_id: leadId }),
  ...
  if (commercialResult.error || conversationResult.error || activitiesResult.error || notesResult.error || tasksResult.error) {
  ...
  return {
    ...
    commercialProfile: Object.keys((commercialResult.data ?? {}) as Record<string, unknown>).length > 0
      ...
      : null,
    conversation: ((conversationResult.data ?? []) as CrmConversationRow[]).map((message) => ({
